...
globle intBool SubsumeType(
  ...
  int t2)
  {
   if ((t2 == OBJECT_TYPE_CODE) || (t2 == PRIMITIVE_TYPE_CODE))
     return(TRUE);
   if ((t2 == NUMBER_TYPE_CODE) && ((t1 == INTEGER) || (t1 == FLOAT_CLIPS)))
     return(TRUE);
   if ((t2 == LEXEME_TYPE_CODE) && ((t1 == STRING_CLIPS) || (t1 == SYMBOL)))
     return(TRUE);
   ...
   switch (tcode)
     {
      ...
      case FLOAT_CLIPS               : return(FLOAT_TYPE_NAME);
      case SYMBOL              : return(SYMBOL_TYPE_NAME);
      case STRING_CLIPS              : return(STRING_TYPE_NAME);
