...
//(. <task-pattern> <argumets>) - adds task to godot flow array -
...
Variant CLIPS_ENV::get_class_slots(String defclassName, int inheritFlag) {
  DATA_OBJECT temp;
  EnvClassSlots(env,EnvFindDefclass(env,defclassName.ascii()),&temp,inheritFlag);
...
void CLIPS_ENV::make_instance(String makeCommand) {
  EnvMakeInstance(env, makeCommand.ascii());};
...
int CLIPS_ENV::unmake_instance(String instanceName) {
  ...
  void *instancePtr = EnvFindInstance(env,NULL,instanceName.ascii(),FALSE);
...
int CLIPS_ENV::set_slot(String instanceName, String slotName, Variant inputValue, bool as_symbols) {
  ...
  return EnvDirectPutSlot(env,instancePtr,slotName.ascii(),inputDataObject);};


void CLIPS_ENV::assert_string(String string) {
	EnvAssertString(env, string.ascii());};


int CLIPS_ENV::build(String constructString) {
	return EnvBuild(env, constructString.ascii());};


int CLIPS_ENV::load(String filename) {
	return EnvLoad(env, filename.ascii());};
...
Variant CLIPS_ENV::eval(String expressionString) {
  DATA_OBJECT temp;
  EnvEval( env, expressionString.ascii(), &temp);
  return data_object_to_variant(env, &temp);};
