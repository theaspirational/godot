...
static SYMBOL_HN *ParseMethodNameAndIndex(
   ...
   *theIndex = 0;
   ...
   if (GetType(DefgenericData(theEnv)->GenericInputToken) == STRING_CLIPS)
     {
      PPBackup(theEnv);
      PPBackup(theEnv);
      SavePPBuffer(theEnv," ");
...
static EXPRESSION *ValidType(
  ...
  SYMBOL_HN *tname)
  {
   ...
   if (strcmp(ValueToString(tname),FLOAT_TYPE_NAME) == 0)
     return(GenConstant(theEnv,INTEGER,(void *) EnvAddLong(theEnv,(long long) FLOAT_CLIPS)));
   ...
   if (strcmp(ValueToString(tname),STRING_TYPE_NAME) == 0)
     return(GenConstant(theEnv,INTEGER,(void *) EnvAddLong(theEnv,(long long) STRING_CLIPS)));
