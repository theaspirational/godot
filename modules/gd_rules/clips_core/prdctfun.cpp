...
globle intBool LexemepFunction(
  void *theEnv)
  {
   ...
   if ((GetType(item) == SYMBOL) || (GetType(item) == STRING_CLIPS))
     { return(TRUE); }
   else
     { return(FALSE); }
  }
...
globle intBool NumberpFunction(
  void *theEnv)
  {
   ...
   if ((GetType(item) == FLOAT_CLIPS) || (GetType(item) == INTEGER))
     { return(TRUE); }
   else
     { return(FALSE); }
  }
