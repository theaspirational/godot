   ...
   while (theExpression != NULL)
     {
      ...
      if ((theArgument.type == INTEGER) ? (ValueToLong(theArgument.value) == 0L) :
                                 ((theArgument.type == FLOAT_CLIPS) ? ValueToDouble(theArgument.value) == 0.0 : FALSE))
        {
   ...
   while (theExpression != NULL)
     {
      ...
      else if (theArgument.type == FLOAT_CLIPS) theNumber = (long long) ValueToDouble(theArgument.value);
      else theNumber = 1;
...
globle double FloatFunction(
  void *theEnv)
  {
   ...
   if (EnvArgTypeCheck(theEnv,"float",1,FLOAT_CLIPS,&valstruct) == FALSE) return(0.0);
