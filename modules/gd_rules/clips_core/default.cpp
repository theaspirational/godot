...
globle void DeriveDefaultFromConstraints(
  ...
  int garbageMultifield)
  {
   ...
   if (constraints->anyAllowed || constraints->symbolsAllowed)
     ...
     {
      theType = STRING_CLIPS;
      theValue = FindDefaultValue(theEnv,STRING_CLIPS,constraints,EnvAddSymbol(theEnv,""));
     }
     ...
     {
      theType = FLOAT_CLIPS;
      theValue = FindDefaultValue(theEnv,FLOAT_CLIPS,constraints,EnvAddDouble(theEnv,0.0));
     }
#if OBJECT_SYSTEM
   ...
   if (theType == INTEGER)
     {
      ...
      else if (theConstraints->minValue->type == FLOAT_CLIPS)
      ...
      else if (theConstraints->maxValue->type == FLOAT_CLIPS)
        { return(EnvAddLong(theEnv,(long long) ValueToDouble(theConstraints->maxValue->value))); }
     }
   else if (theType == FLOAT_CLIPS)
     {
      if (theConstraints->minValue->type == FLOAT_CLIPS)
        { return(theConstraints->minValue->value); }
