...
globle int EnvArgTypeCheck(
  ...
  DATA_OBJECT_PTR returnValue)
  {
   ...
   if ((expectedType == INTEGER_OR_FLOAT) &&
       ((returnValue->type == INTEGER) || (returnValue->type == FLOAT_CLIPS)))
     { return(TRUE); }

   if ((expectedType == SYMBOL_OR_STRING) &&
       ((returnValue->type == SYMBOL) || (returnValue->type == STRING_CLIPS)))
     { return(TRUE); }
   ...
   if ((returnValue->type == INTEGER) && (expectedType == FLOAT_CLIPS))
     {
      returnValue->type = FLOAT_CLIPS;
      returnValue->value = (void *) EnvAddDouble(theEnv,(double) ValueToLong(returnValue->value));
      return(TRUE);
     }

   if ((returnValue->type == FLOAT_CLIPS) && (expectedType == INTEGER))
     {
   ...
   if (expectedType == FLOAT_CLIPS) ExpectedTypeError1(theEnv,functionName,argumentPosition,"float");
   ...
   else if (expectedType == STRING_CLIPS) ExpectedTypeError1(theEnv,functionName,argumentPosition,"string");
...
globle intBool GetNumericArgument(
  ...
  int whichArgument)
  {
   ...
   if ((theType != FLOAT_CLIPS) && (theType != INTEGER))
     {
...
globle const char *GetLogicalName(
  ...
  const char *defaultLogicalName)
  {
   ...
   else if (GetType(result) == FLOAT_CLIPS)
     {
...
globle const char *GetFileName(
  ...
  int whichArgument)
  {
   ...
   EnvRtnUnknown(theEnv,whichArgument,&result);
   if ((GetType(result) != STRING_CLIPS) && (GetType(result) != SYMBOL))
     {
