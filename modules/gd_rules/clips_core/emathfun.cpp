...
static int SingleNumberCheck(
  ...
  double *theNumber)
  {
   ...
   if (EnvArgTypeCheck(theEnv,functionName,1,FLOAT_CLIPS,&theValue) == FALSE) return(FALSE);
...
globle double PowFunction(
  void *theEnv)
  {
   ...
   if (EnvArgTypeCheck(theEnv,"**",1,FLOAT_CLIPS,&value1) == FALSE) return(0.0);
   if (EnvArgTypeCheck(theEnv,"**",2,FLOAT_CLIPS,&value2) == FALSE) return(0.0);
...
globle void ModFunction(
  ...
  DATA_OBJECT_PTR result)
  {
   ...
   if (((item2.type == INTEGER) ? (ValueToLong(item2.value) == 0L) : FALSE) ||
       ((item2.type == FLOAT_CLIPS) ? ValueToDouble(item2.value) == 0.0 : FALSE))
     {
