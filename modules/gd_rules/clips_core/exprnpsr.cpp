  ...
  const char *name)
  {
   ...
   if ((position = FindModuleSeparator(name)) != FALSE)
     { 
      ...
      if (moduleName == NULL)
        {
         PrintErrorID(theEnv,"EXPRNPSR",8,TRUE);
         EnvPrintRouter(theEnv,WERROR,"Missing module name for '");
...
globle struct expr *ArgumentParse(
  ...
  int *errorFlag)
  {
   ...
   if ((theToken.type == SF_VARIABLE) || (theToken.type == MF_VARIABLE) ||
       (theToken.type == SYMBOL) || (theToken.type == STRING_CLIPS) ||
...
#endif
       (theToken.type == FLOAT_CLIPS) || (theToken.type == INTEGER))
...
globle struct expr *ParseAtomOrExpression(
  ...
  struct token *useToken)
  {
   ...
   if ((thisToken->type == SYMBOL) || (thisToken->type == STRING_CLIPS) ||
       (thisToken->type == INTEGER) || (thisToken->type == FLOAT_CLIPS) ||
#if OBJECT_SYSTEM
   ...
   while (TRUE)
     {
      ...
      else if ((theToken->type == SYMBOL) || (theToken->type == STRING_CLIPS) ||
          (theToken->type == INTEGER) || (theToken->type == FLOAT_CLIPS) ||
#if DEFGLOBAL_CONSTRUCT
          (theToken->type == GBL_VARIABLE) ||
          (theToken->type == MF_GBL_VARIABLE) ||
#endif
...
globle EXPRESSION *ParseConstantArguments(
   ...
   *error = FALSE;
   ...
   while (tkn.type != STOP)
     {
      if ((tkn.type != SYMBOL) && (tkn.type != STRING_CLIPS) &&
          (tkn.type != FLOAT_CLIPS) && (tkn.type != INTEGER) &&
