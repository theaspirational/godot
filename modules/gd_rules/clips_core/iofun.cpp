...
globle void ReadFunction(
  ...
  DATA_OBJECT_PTR returnValue)
  {
   ...
   if ((theToken.type == FLOAT_CLIPS) || (theToken.type == STRING_CLIPS) ||
#if OBJECT_SYSTEM
...
globle int OpenFunction(
  void *theEnv)
  {
   ...
   if (numberOfArguments == 2)
     ...
     {
      if (EnvArgTypeCheck(theEnv,"open",3,STRING_CLIPS,&theArgument) == FALSE) return(0);
...
static const char *ControlStringCheck(
  ...
  int argCount)
  {
   ...
   if (EnvArgTypeCheck(theEnv,"format",2,STRING_CLIPS,&t_ptr) == FALSE) return(NULL);
   ...
   switch (formatType)
     {
      ...
      case 'c':
        ...
        if ((GetType(theResult) == STRING_CLIPS) ||
      ...
      case 'u':
        ...
        if (GetType(theResult) == FLOAT_CLIPS)
...
globle void SetLocaleFunction(
  ...
  DATA_OBJECT_PTR returnValue)
  {
   ...
   if (EnvArgTypeCheck(theEnv,"set-locale",1,STRING_CLIPS,&theResult) == FALSE)
     {
      returnValue->type = SYMBOL;
      returnValue->value = EnvFalseSymbol(theEnv);
      return;
