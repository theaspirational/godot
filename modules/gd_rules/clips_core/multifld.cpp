...
globle struct multifield *StringToMultifield(
  ...
  const char *theString)
  {
   ...
   while (theToken.type != STOP)
     {
      if ((theToken.type == SYMBOL) || (theToken.type == STRING_CLIPS) ||
          (theToken.type == FLOAT_CLIPS) || (theToken.type == INTEGER) ||
          (theToken.type == INSTANCE_NAME))
        { theAtom = GenConstant(theEnv,theToken.type,theToken.value); }
      else
        { theAtom = GenConstant(theEnv,STRING_CLIPS,EnvAddSymbol(theEnv,theToken.printForm)); }
...
globle void *ImplodeMultifield(
  ...
  DATA_OBJECT *value)
  {
   ...
   for (i = GetpDOBegin(value) ; i <= GetpDOEnd(value) ; i++)
     {
      if (GetMFType(theMultifield,i) == FLOAT_CLIPS)
        {
      ...
      else if (GetMFType(theMultifield,i) == STRING_CLIPS)
        {
